<div class="mb-3 w-100 px-lg-5 px-md-3 px-2  min-h-[75vh]">
    ...
    <div class="alert alert-info d-flex justify-content-between align-items-center mb-3">
        ...
        <div>
            ...
            <button wire:click="clearFilter" class="btn btn-sm btn-outline-primary">
                <i class="fa-solid fa-times"></i> Show All Family Planning Records
            </button>
        </div>
    </div>
    @endif
    <div class="filters d-flex justify-content-lg-between  justify-content-end  flex-wrap flex-xl-nowrap gap-3 mb-2 mb-md-0">
        ...
        <div class="button-con d-flex align-items-center mt-1 gap-1">
            {{-- Download PDF (red) --}}
            <button wire:click="exportPdf" type="button"
                ...
                <p class="mb-0" style="font-size: 0.875rem;">PDF</p>
            </button>
            ...
            <button wire:click="exportExcel" type="button"
                ...
                <p class="mb-0" style="font-size: 0.875rem;">Excel</p>
            </button>
            <a href="{{route('record.archive')}}?type_of_case=family-planning" class="btn btn-danger">
    ...
    <div class="tables table-responsive">
        <table class="w-100 table">
            ...
            <!-- data of patient -->
            <tbody>
                @forelse($familyPlanningRecords as $record)
                <tr class="{{ isset($record->followup_status_info['class']) ? $record->followup_status_info['class'] : '' }}">
                    <td>{{ $familyPlanningRecords->firstItem() + $loop->index }}</td>
                    ...
                    <!-- NEW: Status Column -->
                    <td>
                        @if(isset($record->followup_status_info) && is_array($record->followup_status_info))
                        <span class="{{ $record->followup_status_info['badge_class'] ?? 'badge bg-secondary' }}">
                            {{ $record->followup_status_info['badge'] ?? 'No Status' }}
                        </span>
                        @if(isset($record->followup_status_info['followup_date']))
                        <div class="text-muted small mt-1">
                            Due: {{ $record->followup_status_info['followup_date'] }}
                        </div>
                        @endif
                        @else
                    ...
                    <td>{{$record->date_of_registration?->format('M d, Y') ?? 'N/A' }}</td>
                    <td>
                        <div class="actions d-flex gap-2 justify-content-center align-items-center">
                            ...
                            @endphp
                            <a href="/patient-record/family-planning/view-detail/{{ $record->id }}?{{$backParams}}">
                            ...
                            <a href="/patient-record/family-planning/edit-details/{{ $record->id }}?{{$backParams}}" class=" text-success fw-bold fs-3">
                                <i class="fa-solid fa-pen-to-square"></i>
                            </a>
                            <a href="" class="delete-record-icon-familyPlanning fs-2" data-bs-patient-id="{{ $record->patient->id }}">
                                <i class="fa-solid fa-box-archive text-danger"></i>
                            </a>
                            <a href="/patient-record/family-planning/view-case/{{ $record->id }}?{{$backParams}}" class=" text-warning fw-bold fs-3">
