...
saveBtn.addEventListener("click", async (e) => {
    ...
    try {
        ...
        const form = document.getElementById("edit-general-consultation-form");
        const formData = new FormData(form);

        const response = await fetch(
            `/update/general-consultation/details/${id}`,
            {
                method: "POST",
                headers: {
        ...
        if (!response.ok) {
            // reset the error elements first
            ...
            await Swal.fire({
                title: "Update General Consultation Patient Details",
